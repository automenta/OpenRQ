...
final class ISDManager {
    ...
    private static final Pattern K_PRIME_PATTERN = Pattern.compile(K_PRIME_FORMAT);
    ...
    private static boolean isValidKPrimeLine(String line) {
        ...
        int len = line.length();
        if (len == 0 || len > MAX_K_PRIME_CHARS || line.startsWith("#")) {
            return false;
        }

        return K_PRIME_PATTERN.matcher(line).matches();
    }
