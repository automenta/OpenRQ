...
final class ISDOps {
    ...
    static ISDOperation readOperation(ReadableByteChannel ch) throws IOException {
    ...
    private static final class MatrixVectorMultiplication implements ISDOperation {
        ...
        private static int Dcols(byte[][] D) {

            return (D.length == 0) ? 0 : D[0].length;
        }

        private static byte[][] DShallowCopy(byte[][] D) {

            return Arrays.copyOf(D, D.length);
        }

        private static byte[] getInnerArray(ByteVector v) {
