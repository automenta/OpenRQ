...
public final class ArrayIO {
    ...
    public static void writeInts(WritableByteChannel ch, int[] src, int from, int to) throws IOException {
        ...
        while (off < to) {
            ...
            writeFromArrayRegion(ch, src, off, len, buf);
            off += len;
        }
    }
    ...
    public static void readInts(ReadableByteChannel ch, int[] dst, int from, int to) throws IOException {
        ...
        while (off < to) {
            final int remaining = to - off;
            final int len = Math.min(remaining, BUFFER_SIZE / SizeOf.INT);
            readToArrayRegion(ch, dst, off, len, buf);
            off += len;
        }
    }
     ...
     */
    private static void writeFromArrayRegion(WritableByteChannel ch, int[] array, int off, int len, ByteBuffer buf) throws IOException {
     ...
     * requires len <= BUFFER_SIZE / SizeOf.INT
     */
    private static void readToArrayRegion(ReadableByteChannel ch, int[] array, int off, int len, ByteBuffer buf) throws IOException {
    ...
    private static final ThreadLocal<ByteBuffer> CACHED_BUFFER = ThreadLocal.withInitial(() -> ByteBuffer.allocate(BUFFER_SIZE));
    private static final ThreadLocal<ByteBuffer> CACHED_DIRECT_BUFFER = ThreadLocal.withInitial(() -> ByteBuffer.allocateDirect(BUFFER_SIZE));
